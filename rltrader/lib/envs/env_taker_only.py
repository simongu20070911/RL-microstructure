...
class HFTEnv(gym.Env):
    ...
    def _validate_config(self):
        ...
        required_keys = [
            ...
            "taker_penalty"  # NEW: Penalty factor when an order executes as a taker
        ]
            ...
            try:
                ...
                if not all(bids[i, 0] >= bids[i+1, 0] for i in range(len(bids)-1)) or \
                   ...
                   (len(bids) > 0 and len(asks) > 0 and bids[0,0] >= asks[0,0]):
                    logging.warning(f"Order book structure violation at row index {row_count}. Skipping row.")
                    continue
    ...
    def _create_observation_space(self):
        ...
        obs_size = (
            ...
            2 * self.config["max_active_orders"] +  # order prices and volumes (normalized)
            4 * self.config["order_book_levels"] +  # bid/ask prices and volumes (normalized)
            1  # spread (normalized)
        )
    ...
    def _update_market_state(self):
        ...
        if len(self.bids) == 0 or len(self.asks) == 0:
            logging.warning(f"Step {self.current_step}: Empty bids or asks encountered. Using defaults.")
        ...
        if self.best_bid >= self.best_ask:
            logging.warning(f"Step {self.current_step}: Order book crossed (Bid: {self.best_bid}, Ask: {self.best_ask}).")
    ...
    def step(self, action):
        ...
        if terminated or truncated:
            logging.info(f"Episode end condition met. Liquidating positions.")
        ...
        logging.debug(f"Step {self.current_step} - Reward: {reward:.4f}")
        ...
        observation = self._get_observation()
        info = self._get_info()
        logging.debug(f"Step {self.current_step} End - MTM: {info['mtm']:.2f}")
    ...
    def _place_new_order(self, is_buy, price_offset, volume_scaled):
        """Places a new order, applying penalties if the order price would immediately cross the spread.
        
        Note: Although the order is placed regardless, a penalty is applied if the order would be a taker.
        """
        penalty = 0.0
        ...
        # Optional warning if the order would execute immediately (taker)
        if is_buy and limit_price >= self.best_ask:
            logging.debug(f"Buy order price {limit_price} would execute immediately (>= best ask {self.best_ask}).")
            penalty -= self.config["invalid_order_penalty"]
        if not is_buy and limit_price <= self.best_bid:
            logging.debug(f"Sell order price {limit_price} would execute immediately (<= best bid {self.best_bid}).")
            penalty -= self.config["invalid_order_penalty"]
        ...
        for order in self.pending_orders:
            if self.current_step >= order["target_step"]:
                if len(self.active_orders) < self.config["max_active_orders"]:
                    ...
                    logging.warning(f"Order {order['id']} reached target step but max active orders limit reached. Discarded.")
            else:
    ...
    def _execute_orders(self):
        ...
        for order in self.active_orders:
            ...
            # Check if at execution time the order would be a taker.
            order_taker = False
            if is_buy and limit_price >= self.best_ask:
                order_taker = True
            elif not is_buy and limit_price <= self.best_bid:
                order_taker = True
            ...
            for level_idx in range(len(levels)):
                ...
                if can_fill_at_level:
                    ...
                    if abs(potential_new_inventory) > self.config["max_inventory"]:
                        ...
                        logging.debug(f"Inventory limit breached. Adjusted fill from {fill_qty_possible:.4f} to {fill_qty:.4f}")
                    else:
                    ...
                    # If the order is executing as a taker, apply a penalty per unit executed.
                    if order_taker:
                        taker_penalty = fill_qty * limit_price * self.config["taker_penalty"]
                        realized_pnl -= taker_penalty
                        logging.debug(f"Order {order['id']} is taker: Applying penalty {taker_penalty:.4f}")
                    ...
                    logging.debug(f"Order {order['id']} filled: {fill_qty:.4f} @ {fill_price:.2f}. PNL: {pnl_from_fill:.4f}. Cost: {transaction_cost:.4f}")
    ...
    def _process_fill(self, fill_qty, price, is_buy):
        """Updates positions based on a fill and calculates realized PnL."""
        realized_pnl = 0.0
        if is_buy:
    ...
    def _add_to_long(self, qty, price):
        ...
        if self.long_position <= 1e-9:
            ...
            self.long_avg_cost = ((self.long_avg_cost * self.long_position) + (price * qty)) / new_total_qty
    ...
    def _add_to_short(self, qty, price):
        ...
        if self.short_position <= 1e-9:
            ...
            self.short_avg_cost = ((self.short_avg_cost * self.short_position) + (price * qty)) / new_total_qty
        self.short_position = new_total_qty

    def _calculate_risk_penalty(self):
        """Calculates a penalty based on current net inventory."""
        ...
        penalty = self.config["inventory_penalty"] * (normalized_inventory ** 2) * abs(self.midprice)
        return abs(penalty)
    ...
    def _get_raw_observation(self):
        ...
        order_prices_norm = []
        order_volumes_norm = []
        orders_to_show = sorted(self.active_orders, key=lambda x: x['price'])[:self.config["max_active_orders"]]
    ...
    def _get_observation(self):
        """Gets the current observation, normalized using running stats."""
    ...
    def _update_running_stats(self, obs):
        """Updates the running mean and variance using an exponential moving average."""
    ...
    def _normalize_observation(self, obs):
        ...
        std_dev = np.sqrt(self.running_stats['var'])
    ...
    def render(self, mode="human"):
        ...
        if mode == "human":
            info = self._get_info()
            render_msg = (
                f"\n--- Step: {info['current_step']} / Elapsed: {info['total_steps_elapsed']} ---\n"
    ...
    def _liquidate_positions(self):
        ...
        if np.isnan(best_bid_liq) or np.isnan(best_ask_liq):
            logging.error("Cannot liquidate - market prices unavailable. Setting PnL to 0.")
            return 0.0
...
config = {
    ...
    "taker_penalty": 0.001  # Penalty factor for orders executed as takers
}
