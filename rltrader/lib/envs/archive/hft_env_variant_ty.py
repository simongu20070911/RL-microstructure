...
class HFTEnv(gym.Env):
    def __init__(self, config):
        ...
        # Action space: [signed_volume, price_offset, cancel_fraction]
    ...
    def _update_market_state(self):
        # Ensure bid and ask prices are correctly ordered
    ...
    def _place_new_order(self, is_buy, price_offset, volume_scaled):
        ...
        # Calculate limit price using price_offset's sign
        if is_buy:
            limit_price = self.midprice + (price_offset * self.config["price_offset_ticks"] * self.config["tick_size"])
            # Check if buy order is too aggressive or too passive
            max_allowed = self.best_ask + (self.config["price_offset_ticks"] * self.config["tick_size"])
            min_allowed = self.best_bid - (self.config["price_offset_ticks"] * self.config["tick_size"])
            if limit_price > max_allowed or limit_price < min_allowed:
                penalty -= self.config["invalid_order_penalty"]
                return penalty
        else:
            limit_price = self.midprice - (price_offset * self.config["price_offset_ticks"] * self.config["tick_size"])
            # Check if sell order is too aggressive or too passive
            min_allowed = self.best_bid - (self.config["price_offset_ticks"] * self.config["tick_size"])
            max_allowed = self.best_ask + (self.config["price_offset_ticks"] * self.config["tick_size"])
            if limit_price < min_allowed or limit_price > max_allowed:
                penalty -= self.config["invalid_order_penalty"]
                return penalty
    ...
    def _update_running_stats(self, obs):
        if self.running_stats['mean'] is None:
            ...
            delta = obs - old_mean
            self.running_stats['var'] = self.decay * self.running_stats['var'] + (1 - self.decay) * delta ** 2
            self.running_stats['count'] += 1
