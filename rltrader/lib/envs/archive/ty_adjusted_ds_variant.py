...
class HFTEnv(gym.Env):
    ...
    def _place_new_order(self, is_buy, price_offset, volume_scaled):
        ...
        # Allow the price_offset to affect placement on either side of midprice
        limit_price = self.midprice + (price_offset * self.config["price_offset_ticks"] * self.config["tick_size"])

        # Validate order price relative to market
        if is_buy and limit_price > self.best_ask + (self.config["price_offset_ticks"] * self.config["tick_size"]):
            ...
            return penalty
        elif not is_buy and limit_price < self.best_bid - (self.config["price_offset_ticks"] * self.config["tick_size"]):
            penalty -= self.config.get("invalid_order_penalty", 1.0)
            return penalty
...
config = {
    "csv_path": "/home/gaen/Documents/RL/orderbook.csv",
