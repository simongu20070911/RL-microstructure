...
This environment supports multiple fee structures and post-only trading modes:
1. Standard fees (baseline)
2. Rebated structures (4bps, 6bps, 8bps)  
3. Post-only mode where aggressive orders are auto-rejected
4. Maker-only rebates (only passive liquidity providers get rebates)
"""
...
class RebatedHFTEnv(gym.Env):
    ...
    def __init__(self, config):
        super(RebatedHFTEnv, self).__init__()
        self.config = config
        
        # Set up fee structure and trading mode
    ...
    def _setup_fee_structure(self):
        """Setup fee structure based on configuration."""
        # Get fee structure type
        fee_structure = self.config.get("fee_structure", "baseline")
        post_only = self.config.get("post_only_mode", False)
        
        # Define fee structures
        fee_structures = {
            "baseline": {"long": 0.0001, "short": 0.0001},  # 1 bps fee
            "rebate_4bps": {"long": -0.00004, "short": -0.00004},  # 4 bps rebate
            "rebate_6bps": {"long": -0.00006, "short": -0.00006},  # 6 bps rebate  
            "rebate_8bps": {"long": -0.00008, "short": -0.00008},  # 8 bps rebate
        }
        
        if fee_structure not in fee_structures:
            raise ValueError(f"Invalid fee_structure: {fee_structure}. Must be one of {list(fee_structures.keys())}")
        
        # Apply fee structure
        fees = fee_structures[fee_structure]
        self.config["transaction_cost_long"] = fees["long"]
        self.config["transaction_cost_short"] = fees["short"]
        
        # Store fee structure info
        self.fee_structure = fee_structure
        self.post_only_mode = post_only
        self.is_rebated = fee_structure.startswith("rebate")
        
        # Initialize volume tracking for ALL environments
        self.maker_volume = 0.0
        self.taker_volume = 0.0
        
        # Initialize rebate-specific tracking for rebated structures
        if self.is_rebated:
            self.total_rebates_earned = 0.0
            self.rebate_rate = abs(fees["long"])  # Convert negative to positive for tracking
        else:
            # Initialize for baseline environments too
            self.total_rebates_earned = 0.0
            self.rebate_rate = 0.0
        
        # Adjust aggressiveness for post-only mode
        if post_only:
            self.config["allowed_aggressiveness_ticks"] = 0  # No aggressive orders allowed
        
        logging.info(f"Fee structure: {fee_structure}, Post-only: {post_only}, "
                    f"Transaction costs: {fees['long']:.6f}/{fees['short']:.6f}")
    ...
    def _load_order_book_data(self):
        ...
        # Check for timestamp column (try multiple variations)
        ...
        numeric_cols = required_columns  # All columns except timestamp are numeric 
    ...
    def reset(self, *, seed: int | None = None, options: dict | None = None):
        ...
        Crucially, allows continuation through the dataset by not resetting
        `self.current_step` unless the end of the data is reached.
        
        Options:
        - preserve_memory: If True, agent memory (LSTM states and attention cache) 
          will be preserved across episodes for sequential market playback.
        """
        super().reset(seed=seed)
        
        # Check if we should preserve agent memory across episodes
        preserve_memory_option = options.get('preserve_memory', True) if options else True
        # Validate preserve_memory type
        if not isinstance(preserve_memory_option, bool):
            logging.warning(f"Invalid preserve_memory type {type(preserve_memory_option)}, defaulting to True")
            preserve_memory_option = True
        self.preserve_memory = preserve_memory_option
        logging.info(f"Episode reset with preserve_memory={self.preserve_memory}")
        ...
        # Reset rebate tracking variables for new episode
        self.total_rebates_earned = 0.0
        self.maker_volume = 0.0
        self.taker_volume = 0.0

        # --- 3. Load Market State for the NEW Current Step ---
        try:
        ...
        # --- 4. Get Initial Observation and Info ---
        ...
        logging.info(f"Reset complete. Start Step: {self.current_step}. Initial MTM: {info.get('mtm', 'N/A'):.2f}. "
                    f"Obs Shape: {observation.shape}")

        # --- 5. Final Observation Validation ---
        if not self.observation_space.contains(observation):
             ...
             if observation.shape != self.observation_space.shape:
                 ...
                 return np.zeros_like(observation), info
    ...
    def step(self, action):
        ...
        if do_nothing_triggered:
            ...
            # FIX: Skip order placement if cancellation was triggered
            if explicit_cancel_triggered:
                logging.debug("Skipping order placement due to explicit cancellation")
                buy_volume_scaled = 0.0  # No orders when cancelling
                sell_volume_scaled = 0.0
            else:
        ...
        liquidation_pnl = 0.0
        liquidation_mode = self.config.get('liquidation_mode', 'always')
        
        if terminated or truncated:
            reasons = []
            risk_liquidation_reasons = []
            natural_end_reasons = []
            
            if terminated_cash: 
                reasons.append("Cash<0")
                risk_liquidation_reasons.append("Cash<0")
            if terminated_inv: 
                reasons.append("InvLimit")
                risk_liquidation_reasons.append("InvLimit")
            if terminated_data: 
                reasons.append("EndData")
                natural_end_reasons.append("EndData")
            if truncated_len: 
                reasons.append("MaxEpLen")
                natural_end_reasons.append("MaxEpLen")
            
            should_liquidate = False
            if liquidation_mode == 'always':
                # Original behavior: always liquidate
                should_liquidate = True
                logging.info(f"Episode end ({', '.join(reasons)}). Term={terminated}, Trunc={truncated}. Liquidating (mode: always).")
            elif liquidation_mode == 'risk_based':
                # Only liquidate for risk management reasons
                should_liquidate = bool(risk_liquidation_reasons)
                if should_liquidate:
                    logging.info(f"Episode end ({', '.join(reasons)}). Risk-based liquidation triggered by: {', '.join(risk_liquidation_reasons)}.")
                else:
                    logging.info(f"Episode end ({', '.join(reasons)}). No liquidation (natural end: {', '.join(natural_end_reasons)}).")
            elif liquidation_mode == 'never':
                # Never liquidate (for testing)
                should_liquidate = False
                logging.info(f"Episode end ({', '.join(reasons)}). No liquidation (mode: never).")
        ...
        observation = self._get_observation()
        info = self._get_info()  # Get info BEFORE using it for logging
        ...
        if math.isnan(midprice_mtm) or midprice_mtm <=0 : midprice_mtm = 1.0
        ...
        # Add liquidation PnL explicitly to info for tracking
        info['liquidation_pnl'] = liquidation_pnl if (terminated or truncated) else 0.0
        ...
        # --- Advance Market Data or Increment Step Counter if Episode Ended ---
        if terminated or truncated:
            if self.current_step < len(self.order_book_history):
                self.current_step += 1
            logging.debug(f"Episode ended. Final current_step before reset call: {self.current_step}")
        else:
            self._update_step_state()

        # Log final state summary (now we have info)
        ...
        step_num = self.current_step - 1 if (terminated or truncated) else self.current_step
        logging.debug(f"Step {step_num} End State{term_trunc_flag} - MTM: {log_mtm:.2f}, Inv: {final_inv_log:.4f}, Cash: {log_cash:.2f}, Long@{log_long_cost:.2f}, Short@{log_short_cost:.2f}")
    ...
    def _place_single_order(self, is_buy, price_offset_action, volume_scaled):
        ...
        # In post-only mode, reject orders that would immediately execute (become takers)
        if self.post_only_mode:
            is_aggressive = False
            if is_buy and target_limit_price >= best_ask_eff - 1e-9:
                is_aggressive = True
            elif not is_buy and target_limit_price <= best_bid_eff + 1e-9:
                is_aggressive = True
            
            if is_aggressive:
                logging.debug(f"Post-Only REJECTION: {'Buy' if is_buy else 'Sell'} at {target_limit_price:.2f} would be aggressive (BBO: {best_bid_eff:.2f}/{best_ask_eff:.2f})")
                return False  # Order rejected - would be taker
        
        # --- Aggressiveness Clipping ---
        if self.post_only_mode and max_aggressive_placement_ticks == 0:
            # In post-only mode, prevent any order from crossing the spread
            # Buy orders must be at or below best bid (not at or above ask)
            # Sell orders must be at or above best ask (not at or below bid)
            max_allowed_aggressive_buy_price = clip_bid_ref  # Max buy = best bid
            min_allowed_aggressive_sell_price = clip_ask_ref  # Min sell = best ask
        else:
            # Normal mode: allow crossing spread up to aggressiveness limit
        ...
        order = {
            ...
            "placement_bid": self.best_bid,  # *** FIX: Store BBO at placement time for correct rebate calculation ***
            "placement_ask": self.best_ask   # *** FIX: Store BBO at placement time for correct rebate calculation ***
        }
    ...
    def _process_pending_orders(self):
        ...
        # --- Get BBO at the current step (activation time) for fallback use only ---
        ...
        for order in self.pending_orders:
            ...
            if self.current_step >= order["target_step"]:
                ...
                if len(self.active_orders) < max_active:
                    # *** FIX: Calculate Taker Status Based on BBO at PLACEMENT Time ***
                    # Use stored placement BBO to determine if order was providing liquidity when posted
                    placement_bid = order.get("placement_bid", best_bid_activation)
                    placement_ask = order.get("placement_ask", best_ask_activation)
                    
                    is_taker_at_placement = False
                    taker_reason = "Maker"
                    if order["is_buy"] and order["price"] >= placement_ask - 1e-9:
                        is_taker_at_placement = True
                        taker_reason = f"Buy Price {order['price']:.2f} >= Ask@Placement {placement_ask:.2f}"
                    elif not order["is_buy"] and order["price"] <= placement_bid + 1e-9:
                        is_taker_at_placement = True
                        taker_reason = f"Sell Price {order['price']:.2f} <= Bid@Placement {placement_bid:.2f}"

                    # Store the calculated status in the order dictionary (keep old name for compatibility)
                    order["is_taker_at_activation"] = is_taker_at_placement

                    # Move to active list
                    self.active_orders.append(order)
                    activated_count += 1
                    logging.debug(f"  Activated Order {order['id']} ({'Buy' if order['is_buy'] else 'Sell'} {order['volume']:.4f} @ {order['price']:.2f}). Taker@Placement: {is_taker_at_placement} ({taker_reason})")
            ...
            for level_idx in range(levels_to_match.shape[0]):
                ...
                if can_fill_at_level:
                    ...
                    if is_buy and potential_new_inventory > max_inv + 1e-9:
                        fill_qty = max(0.0, min(fill_qty_possible, max_inv - current_net_inventory)) # Fill only up to the limit
                        inventory_limit_hit = True
                    elif not is_buy and potential_new_inventory < -max_inv - 1e-9:
                        fill_qty = max(0.0, min(fill_qty_possible, current_net_inventory + max_inv)) # Fill only down to the limit
                        inventory_limit_hit = True
                    ...
                    if fill_qty > 1e-9:
                        # --- Update Cash & Apply Costs/Rebates ---
                        # FIXME: Transaction cost calculation should happen AFTER inventory limits
                        ...
                        if self.is_rebated and is_maker:
                            # Maker order in rebated environment - GIVE REBATE
                            rebate = self.rebate_rate * executed_value
                            self.cash += rebate  # ADD rebate money
                            self.total_rebates_earned += rebate
                            self.maker_volume += fill_qty
                            transaction_cost = -rebate  # Track as negative cost
                            logging.debug(f"    REBATE Applied: ${rebate:.6f} for maker order {order_id}")
                        else:
                            # Taker order or non-rebated - CHARGE FEE
                            transaction_cost = cost_rate * executed_value
                            self.cash -= transaction_cost
                            
                            # Track volume based on maker/taker status
                            if is_maker:
                                self.maker_volume += fill_qty
                            else:
                                self.taker_volume += fill_qty
    ...
    def _get_raw_observation(self):
        ...
        spread_norm_ticks = max(0.0, spread_safe / norm_ref_price_tick) / obs_price_scale # Ensure non-negative
    ...
    def _get_info(self):
        ...
        info = {
            ...
            "total_rebates_earned": getattr(self, 'total_rebates_earned', 0.0),
            "maker_volume": getattr(self, 'maker_volume', 0.0),
            "taker_volume": getattr(self, 'taker_volume', 0.0),
            "rebated_volume": getattr(self, 'maker_volume', 0.0),  # Rebated volume = maker volume
            "is_rebated": getattr(self, 'is_rebated', False),
            "rebate_rate": getattr(self, 'rebate_rate', 0.0),
    ...
    def _liquidate_positions(self):
        ...
        if self.long_position > 1e-9:
            ...
            value = qty * price
            # FIX: No transaction costs for forced liquidation
            cost = 0.0  # Forced liquidation should not incur transaction costs
            ...
            net_pnl = pnl_gross  # No transaction costs for forced liquidation

            self.cash += value  # Add full value received, no costs deducted
        ...
        if self.short_position > 1e-9:
            ...
            self.cash -= value  # Subtract only value paid, no additional costs
            liquidation_pnl_net += net_pnl
            total_liq_cost += cost
        ...
        # Log rebate information and final episode PnL including liquidation
        total_rebates = getattr(self, 'total_rebates_earned', 0.0)
        maker_vol = getattr(self, 'maker_volume', 0.0)
        taker_vol = getattr(self, 'taker_volume', 0.0)
        final_episode_pnl = self.cash - self.config["initial_capital"]
        ...
        logging.info(f"Episode Rebates: ${total_rebates:.4f} | Maker Vol: {maker_vol:.2f} | Taker Vol: {taker_vol:.2f}")
        logging.info(f"Final Episode PnL (including liquidation): ${final_episode_pnl:.2f} | Trading PnL: ${final_episode_pnl - total_rebates:.2f} | Rebate PnL: ${total_rebates:.2f}")
...
def get_unified_config(fee_structure="baseline", post_only_mode=False):
    """
    Get configuration for unified rebated environment.
    
    Args:
        fee_structure: "baseline", "rebate_4bps", "rebate_6bps", or "rebate_8bps"
        post_only_mode: Whether to enable post-only trading (aggressive orders rejected)
    """
    config = {
        ...
        # Unified environment specific settings
...
if __name__ == '__main__':
    # Test all fee structures and post-only mode
    test_configs = [
        ...
        ("4 bps Rebate Post-Only", "rebate_4bps", True),
    ...
    for name, fee_structure, post_only in test_configs:
        ...
        try:
            ...
            for step in range(5):
                ...
                print(f"Step {step}: Reward={reward:.4f}, MTM={info.get('mtm', 0):.2f}")
                
                # Print rebate info if available
                if env.is_rebated:
                    rebates = info.get('total_rebates_earned', 0.0)
                    volume = info.get('rebated_volume', 0.0)
                    print(f"  Rebates: ${rebates:.6f}, Volume: {volume:.4f}")
    ...
    print("\n" + "="*60)
    print("UNIFIED REBATED ENVIRONMENT TEST COMPLETE")
    print("="*60)