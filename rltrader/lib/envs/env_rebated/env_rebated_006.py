# -*- coding: utf-8 -*-
"""
HFT Environment with 0.006% Rebate (6 bps)

This environment extends the base HFT environment to simulate market maker rebates
of 0.006% (6 basis points). This rebate rate corresponds to very high-volume VIP tiers
on exchanges like Binance VIP 7-8 level.

Key changes from base environment:
- transaction_cost_long: -0.00006 (negative = rebate)
- transaction_cost_short: -0.00006 (negative = rebate)
"""
...
class HFTEnvRebated006(HFTEnv):
    """
    HFT Environment with 0.006% rebate for market making activities.
    ...
    def __init__(self, config):
        ...
        # Apply 0.006% rebate (6 bps) - negative transaction costs
        rebated_config["transaction_cost_long"] = -0.00006   # 6 bps rebate for buys
        rebated_config["transaction_cost_short"] = -0.00006  # 6 bps rebate for sells
        
        # Store rebate information for tracking
        self.rebate_rate = 0.00006
        self.rebate_bps = 6
    ...
    def step(self, action):
        """Override step to track rebate earnings."""
        # Store pre-step cash to calculate rebate earnings
        pre_step_cash = self.cash
        
        # Call parent step method
        observation, reward, terminated, truncated, info = super().step(action)
        
        # Calculate rebates earned this step (cash increase from negative transaction costs)
        cash_change = self.cash - pre_step_cash
        
        # Update rebate tracking (only positive cash changes from transactions)
        if hasattr(self, 'last_executed_volume') and self.last_executed_volume > 0:
            step_rebates = self.last_executed_volume * self.rebate_rate * self.midprice
            self.total_rebates_earned += step_rebates
            self.rebated_volume += self.last_executed_volume
...
def get_default_rebated_config_006():
    """
    Returns a default configuration for 0.006% rebated environment.
    ...
    config = {
        ...
        # Rebated Transaction Costs (6 bps rebate)
        "transaction_cost_long": -0.00006,   # 6 bps rebate for buys
        "transaction_cost_short": -0.00006,  # 6 bps rebate for sells
...
if __name__ == "__main__":
    ...
    print("Testing HFTEnvRebated006 Environment")
