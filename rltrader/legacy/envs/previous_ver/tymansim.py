...
class HFTEnv(gym.Env):
    ...
    def _process_latency_queue(self):
        """Process orders in the latency queue"""
        # Process the oldest order (if any)
...
config = {
    "csv_path": "/home/gaen/Documents/billions/RL/data/orderbook.csv",  # Path to order book data file
    #"csv_path": "/home/gaen/Documents/RL/orderbook_trimmed.csv",  # Alternate CSV path
    "initial_capital": 20000.0,    # Quote currency (e.g., USDT)
    "max_steps": 10_000,           # Number of timesteps
    "order_book_levels": 9,        # Number of price levels to consider
    "price_offset_ticks": 15,      # Maximum price deviation in number of ticks
    "max_order_volume": 100.0,     # Maximum order size in base currency (e.g., ETH)
    "latency_steps": 1,            # Number of timesteps for order processing delay
    "tick_size": 0.01,             # Minimum price increment in quote currency
    "lot_size": 0.001,             # Minimum trade size in base currency (e.g., ETH)
    "max_active_orders": 5,        # Maximum number of open orders
    "inventory_penalty": 0.000,   # Risk penalty coefficient (per unit of base currency)
    "transaction_cost": 0.000,     # Fee as fraction of traded value
    "max_inventory": 5,            # Maximum position size in base currency (e.g., ETH)
    "invalid_order_penalty": 1.0,  # Penalty for placing an order outside the clipped zone
    "activity_bonus": 0.000        # Bonus factor per unit executed volume to encourage active trading
}
