...
\n\n
\begin{abstract}
This paper presents the complete research journey of developing reinforcement learning strategies for high-frequency trading, documenting both the successes and failures encountered across 157 experiments conducted over several months. Our journey reveals a critical insight: only 14 experiments (8.9%) met the reliability criteria for production deployment, while 4 experiments achieved unrealistic profits (including one with 71,006 PnL) due to missing transaction cost implementation. This comprehensive analysis serves as both a methodological contribution and a cautionary tale for the financial RL community. We document the evolution from unrealistic experimental setups that produced misleading results to rigorous validation protocols that ensure practical applicability. Key contributions include: (1) a complete taxonomy of data quality issues in financial RL research, (2) insights from "unrealistic" experiments that reveal market microstructure effects, (3) a validated methodology for ensuring experimental realism, and (4) practical guidelines for transitioning from research to production. Our findings demonstrate that while RL can learn profitable trading strategies, the path from promising research results to production deployment requires rigorous validation that most current research overlooks.
\end{abstract}
\n\n
...
\textbf{Legacy Experiments (prev\_loggged)}: Our early experiments (156 total) represent the "optimistic phase" of our research. These experiments achieved seemingly impressive results with mean PnL of 1528.4 and maximum PnL of 71006.5. However, detailed analysis revealed that many lacked proper transaction cost implementation, leading to unrealistic performance estimates.
...
Our analysis identified 4 experiments with unrealistic performance (PnL > 1,000), with the highest achieving 71,006 PnL and an average of 30,477.3 PnL. These results, while initially exciting, proved to be artifacts of missing transaction cost implementation.
...
After implementing rigorous validation protocols, we identified 14 experiments meeting all reliability criteria. These experiments show more modest but realistic performance, with mean PnL of -3.63 and best performance of 5.13.
...
Our research involved the development and testing of 16 distinct trading environments, each representing different approaches to modeling realistic trading constraints.
...
This comprehensive research journey, spanning 157 experiments across multiple months, provides a unique perspective on the challenges and opportunities in applying reinforcement learning to high-frequency trading. Our experience illustrates both the promise and the pitfalls of financial RL research.

\subsection{Key Findings}

\subsubsection{The Reality Gap}

Our most significant finding is the substantial gap between research claims and production reality in financial RL. Of our 157 experiments, only 14 (8.9\%) met the rigorous standards necessary for production deployment. This finding has profound implications for the field.
