...
class HFTEnv(gym.Env):
    ...
    In this update the action’s price_offset is mapped into a range that allows
    a slight aggressiveness. For buy orders the mapping is from [best_bid, best_ask + 
    allowed_aggressiveness_ticks * tick_size] and for sell orders from 
    [best_bid - allowed_aggressiveness_ticks * tick_size, best_ask]. The execution 
    logic then cancels any order that would immediately execute (i.e. act as a taker),
    ensuring that final execution is always maker type.
    ...
    def _validate_config(self):
        ...
        required_keys = [
            ...
            "taker_penalty",         # Penalty for taker orders
            "allowed_aggressiveness_ticks"  # NEW: How many ticks of aggressiveness to allow in placement
        ]
    ...
    def reset(self, *, seed: int | None = None, options: dict | None = None):
        ...
        self.active_orders = []
        self.pending_orders = []  # List to store orders waiting due to latency
    ...
    def _place_new_order(self, is_buy, price_offset, volume_scaled):
        """
        Places a new order by mapping the action's price_offset into a range that allows
        a tunable level of aggressiveness. For buy orders, the mapping is from:
            [best_bid, best_ask + allowed_aggressiveness_ticks * tick_size]
        For sell orders, it is from:
            [best_bid - allowed_aggressiveness_ticks * tick_size, best_ask]
        The execution logic then cancels any order that would immediately cross the spread.
        """
        penalty = 0.0
        ...
        # Compute order volume and adjust by lot size
        ...
        allowed_aggr = self.config["allowed_aggressiveness_ticks"]
        # For buy orders, allow a bit of aggressiveness above best ask.
        if is_buy:
            lower_bound = self.best_bid
            upper_bound = self.best_ask + allowed_aggr * self.config["tick_size"]
        else:
            # For sell orders, allow aggressiveness below best bid.
            lower_bound = self.best_bid - allowed_aggr * self.config["tick_size"]
            upper_bound = self.best_ask

        # Check if the maker range is valid; if not, fallback to best bid/ask.
        if lower_bound > upper_bound:
            logging.warning("Invalid maker range: lower_bound > upper_bound. Using fallback price.")
            limit_price = self.best_bid if is_buy else self.best_ask
        else:
            # Transform price_offset from [-1,1] to [0,1]
            alpha = (price_offset + 1) / 2.0
            limit_price = lower_bound + alpha * (upper_bound - lower_bound)
    ...
    def _execute_orders(self):
        ...
        The maker order check cancels orders that would immediately execute as takers.
        """
        realized_pnl = 0.0
        ...
        for order in self.active_orders:
            ...
            # Maker Order Check: if the order's price would immediately execute, cancel it.
...
config = {
    ...
    "taker_penalty": 0.001,  # Taker penalty per unit volume (should not trigger with maker-only orders)
